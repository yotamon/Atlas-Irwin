-- Paid Growth hardening before rollout.
-- Observations are ingested only through the audited RPC or service role, provider snapshots are
-- idempotent when a provider reference exists, and every audit event belongs to an experiment.

alter table public.paid_growth_events alter column experiment_id set not null;

create unique index paid_growth_observation_provider_ref_idx
  on public.paid_growth_observations(experiment_id, provider_reference)
...
begin
  ...
  values(p_experiment_id,v_owner,v_artist,'paid_growth.observation_recorded','provider',jsonb_build_object(
    'observationId',v_id,
    'verified',p_verified,
