...
values
 ...
 ('37000000-0000-0000-0000-000000000002','27000000-0000-0000-0000-000000000001','Other Artist','other-learning-artist','17000000-0000-0000-0000-000000000001');
...
-- the base score. An approved lower-vocal learning must be strong enough to flip it.
insert into public.moments(
 ...
 (
  ...
  'hook','High vocal Moment','audio','closed-loop-high',array['short_form_hook'],0.70,0.80,0.70,0.90,0.70,0.90,'approved'
 ),
 (
  ...
  'hook','Low vocal Moment','audio','closed-loop-low',array['short_form_hook'],0.70,0.80,0.70,0.10,0.70,0.80,'approved'
 );
...
-- Build two independent content observations in each trait cohort. Explicit
-- moment_id is intentional here: this is historical execution evidence.
...
select is(
  (select count(*)::integer from public.verified_moment_learning_evidence where metric_source='manual'),
  0,
  'manual metrics are visible to analytics but never enter automatic learning evidence'
);
select is(
  (select count(*)::integer from public.marketing_learnings where source='performance'),
  0,
  'manual-only evidence cannot manufacture a learning proposal'
);
...
select is(
  (select count(*)::integer from public.verified_moment_learning_evidence where content_item_id='87000000-0000-0000-0000-000000000012'),
  0,
  'provider-labelled metrics without a provider object id are still untrusted'
);
...
select is(
  (select count(*)::integer from public.verified_moment_learning_evidence where artist_id='37000000-0000-0000-0000-000000000001'),
  4,
  'only explicit Moment to content to provider metric lineage reaches the trusted evidence view'
);
select is(
  (select status from public.marketing_learnings where learning_key='moment-trait:v1:instagram:vocal_score:save_rate:higher'),
  'proposed',
  'verified cohort performance creates a reviewable proposal instead of silently changing behavior'
);
select ok(
  (select evidence_sample_size = 4000 and confidence > 0.55 and expires_at > now()
   from public.marketing_learnings where learning_key='moment-trait:v1:instagram:vocal_score:save_rate:higher'),
  'the proposal carries sample size, confidence and evidence expiry'
);
select ok(
  (select private.is_valid_marketing_learning_effect(effect)
   from public.marketing_learnings where learning_key='moment-trait:v1:instagram:vocal_score:save_rate:higher'),
  'automatic proposals contain only a whitelisted structured decision effect'
);
select is(
  private.refresh_moment_performance_learnings('17000000-0000-0000-0000-000000000001','37000000-0000-0000-0000-000000000002'),
  0,
  'learning refresh is artist-scoped and cannot borrow another artist evidence'
);
...
select is(
  private.is_valid_marketing_learning_effect('{"kind":"prompt_injection","weight":1}'::jsonb),
  false,
  'unknown learning effects are not executable'
);
...
insert into public.content_items(
  ...
  '87000000-0000-0000-0000-000000000002','17000000-0000-0000-0000-000000000001','37000000-0000-0000-0000-000000000001',
  '47000000-0000-0000-0000-000000000001','77000000-0000-0000-0000-000000000001','Learned Moment choice','Instagram','Reel','Saves','planner'
);
select is(
  (select moment_id from public.content_items where id='87000000-0000-0000-0000-000000000002'),
  '67000000-0000-0000-0000-000000000002'::uuid,
  'the next generated creative uses the newly approved learning through the deterministic selector'
);
...
select is(
  (select status from public.marketing_learnings where id='97000000-0000-0000-0000-000000000001'),
  'superseded',
  'approving an opposite conclusion supersedes the older approved rule in the same family'
);
select is(
  (select supersedes_learning_id from public.marketing_learnings where learning_key='moment-trait:v1:instagram:vocal_score:save_rate:higher'),
  '97000000-0000-0000-0000-000000000001'::uuid,
  'the replacement learning retains explicit supersession lineage'
);
select ok(
  private.moment_execution_score('67000000-0000-0000-0000-000000000001','Instagram','Reel','Saves')
    > private.moment_execution_score('67000000-0000-0000-0000-000000000002','Instagram','Reel','Saves'),
...
select is(
  (select count(*)::integer from public.marketing_learnings where artist_id='37000000-0000-0000-0000-000000000002'),
  0,
  'no learning rows leak into another artist scope'
);
