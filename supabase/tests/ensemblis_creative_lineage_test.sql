...
select ok(
  (select moment_id='68000000-0000-0000-0000-000000000001'::uuid and creative_recipe_id is not null
   from public.content_items where id='88000000-0000-0000-000000000001'),
  'generated content carries both an approved Moment and an immutable creative recipe reference'
);
select is(
  (select recipe ->> 'contentAngle' from public.creative_recipes where id=(select creative_recipe_id from public.content_items where id='88000000-0000-0000-000000000001')),
  'vocal cold open',
  'the content recipe captures the creative treatment that caused the execution'
);

create temporary table original_recipe as
select creative_recipe_id as id from public.content_items where id='88000000-0000-0000-000000000001';
update public.content_items set caption='Edited caption' where id='88000000-0000-0000-000000000001';
select isnt((select creative_recipe_id from public.content_items where id='88000000-0000-0000-000000000001'),(select id from original_recipe),'editing a generated treatment creates a new recipe revision instead of rewriting history');
select is((select parent_recipe_id from public.creative_recipes where id=(select creative_recipe_id from public.content_items where id='88000000-0000-0000-000000000001')),(select id from original_recipe),'recipe revisions retain explicit immutable parent lineage');
...
select ok((select creative_recipe_id is not null from public.content_variants where id='89000000-0000-0000-000000000001'),'each creative variant gets its own causal recipe snapshot');
...
insert into public.publication_jobs(
  ...
  '78000000-0000-0000-0000-000000000001','88000000-0000-0000-000000000001','89000000-0000-0000-0000-000000000001',
...
select ok(
  (select moment_id='68000000-0000-0000-0000-000000000001'::uuid
          and creative_recipe_id=(select creative_recipe_id from public.content_variants where id='89000000-0000-0000-000000000001')
   from public.publication_jobs where id='8a000000-0000-0000-000000000001'),
