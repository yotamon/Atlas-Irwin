...
test("track objects have one readable workspace for source, intelligence, stems and lyrics", async () => {
  ...
  for (const snippet of [
    ...
    "<StemIntelligencePanel",
    "<LyricsIntelligencePanel",
    "Detailed track signals",
  ]) assert.ok(track.includes(snippet), `track workspace is missing ${snippet}`);
...
test("release workspace uses the shared object grammar and no Atlas product copy", async () => {
  ...
  for (const stage of ["Select", "Prepare", "Build hype", "Release", "Sustain", "Rediscover", "Produce", "Distribute", "Learn"]) {
    assert.ok(release.includes(stage), `release lifecycle lost ${stage}`);
  }
