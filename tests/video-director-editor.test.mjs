...
test("video editor consumes canonical lyrics and measured stem intelligence through one workspace read model", async () => {
  const [page, loader, editor, stemLane] = await Promise.all([
    source("app/studio/(protected)/video/[id]/page.tsx"),
    source("lib/video-director/workspace.ts"),
    source("components/studio/video-director/editor/video-director-pro-editor.tsx"),
    source("components/studio/video-director/editor/stem-activity-lane.tsx"),
  ]);
  assert.match(page, /loadVideoWorkspaceSnapshot/);
  assert.doesNotMatch(page, /track_lyric_lines|track_stems|audio_scenes/);
  assert.match(loader, /track_lyric_lines/);
  assert.match(loader, /track_stems/);
  assert.match(loader, /audio_scenes/);
  assert.match(loader, /lyricCues/);
  assert.match(loader, /audioScenes/);
...
test("performance shots route through audio-reference-capable continuity models", async () => {
  const policy = await source("lib/video-director/editor-policy.ts");
  ...
  assert.match(policy, /performance_shot/);
  assert.match(policy, /requires_audio_reference/);
  assert.match(policy, /audio_references/);
  assert.match(policy, /performance_mode = "lip_sync"/);
  assert.match(policy, /editor\.shotType === "performance" \? "consistency"/);
...
test("character identity is artist-scoped, reusable and cannot leak across artists", async () => {
  ...
  const policy = await source("lib/video-director/editor-policy.ts");
  const session = await source("lib/video-director/editor-session.ts");
  const workspace = await source("lib/video-director/workspace.ts");
  ...
  assert.match(policy, /artistCharacters/);
  assert.match(policy, /baseReferences/);
  assert.match(session, /assertAllowedArtistReferenceAssets/);
  assert.match(workspace, /music_video_characters.*artist_id/s);
});
...
test("final delivery is fail-closed behind temporal QC and exact human lip-sync identity attestation", async () => {
  ...
  const editorPolicy = await source("lib/video-director/editor-policy.ts");
  ...
  assert.match(humanAction, /buildHumanQualityApprovalMutation/);
  assert.match(editorPolicy, /explicit_human_attestation/);
  assert.match(editorPolicy, /review_asset_id: shot\.selected_asset_id/);
...
test("Source Auto-Edit is explainable, real-first, artist-scoped and non-destructive", async () => {
  ...
  assert.match(session, /isAllowedProjectSourceAsset/);
...
test("Video Director admits reusable artist media without treating sibling releases as global", async () => {
  ...
  assert.match(page, /loadVideoWorkspaceSnapshot/);
  assert.doesNotMatch(page, /projectMediaLinkScopeFilter|\.from\(/);
});
