...
test("Today and Release consume the same release Mission model", async () => {
  ...
  for (const file of [today, release]) {
    assert.ok(file.includes("deriveReleaseMission"));
  }
  assert.ok(today.includes("Active release Mission"));
  assert.ok(today.includes("View release Mission"));
  assert.ok(today.includes('select("id,title,release_date,active_release,artwork_url,cover_asset,primary_hook,smart_link_url,spotify_url,soundcloud_url,youtube_url,status,is_archived")'));
  assert.ok(today.includes('select("id,release_id,audio_url,is_primary")'));
  assert.ok(today.includes('select("id,release_id,status")'));
