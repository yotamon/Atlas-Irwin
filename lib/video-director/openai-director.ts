...
const CONCEPT_SCHEMA = {
  ...
  properties: {
    ...
    world: { type: "string" },
    character_strategy: { type: "string" },
    beginning: { type: "string" },
    middle: { type: "string" },
    ending: { type: "string" },
    musical_fit: { type: "string" },
    complexity: { type: "string", enum: ["low", "medium", "high"] },
    anti_cliches: { type: "array", items: { type: "string" } },
    signature_moments: {
      type: "array",
      items: {
        type: "object",
        additionalProperties: false,
        required: ["time_ms", "description"],
        properties: {
...
const VISUAL_BIBLE_SCHEMA = {
  ...
  properties: {
    ...
    texture_rules: { type: "array", items: { type: "string" } },
...
function outputText(payload: ResponsePayload) {
  ...
  for (const item of payload.output ?? []) {
    for (const content of item.content ?? []) {
      if (content.type === "refusal" && content.refusal) throw new Error(`Creative Director refused: ${content.refusal}`);
...
async function structuredResponse<T>(input: {
  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      model: process.env.VIDEO_DIRECTOR_LLM_MODEL?.trim() || "gpt-5.6",
      store: false,
  ...
  try {
    ...
    throw new Error(`Creative Director returned invalid structured JSON: ${error instanceof Error ? error.message : "unknown parse error"}`);
  }
}
...
function compactContext(context: VideoProjectContext) {
  ...
  return {
    project: {
      ...
      target: context.project.project_kind,
      aspect_ratio: context.project.primary_aspect_ratio,
      resolution: context.project.target_resolution,
      hard_budget_credits: context.project.hard_budget_credits,
      brief,
    },
    track: {
      title: context.track.title,
      duration_seconds: context.track.duration,
      notes: context.track.notes,
    },
    release: {
      title: context.release.title,
      story: context.release.story,
      core_emotion: context.release.core_emotion,
      audience: context.release.audience,
      primary_hook: context.release.primary_hook,
      visual_direction: context.release.visual_direction,
      color_palette: context.release.color_palette,
      release_identity: context.release.release_identity,
      story_answers: context.release.story_answers,
    },
    music_map: context.musicMap,
    brand_settings: context.brandSettings,
    available_media: context.media.map((asset) => ({
      id: asset.id,
      type: asset.asset_type,
      mime: asset.mime_type,
      metadata: asset.metadata,
      has_url: Boolean(asset.public_url),
    })),
  ...
  async createConcepts(context: VideoProjectContext): Promise<VideoConcept[]> {
    const result = await structuredResponse<{ concepts: VideoConcept[] }>({
      ...
      schema: {
        ...
        required: ["concepts"],
        properties: {
          concepts: { type: "array", minItems: 3, maxItems: 3, items: CONCEPT_SCHEMA },
        },
      },
    });
  ...
  async createProductionPlan(context: VideoProjectContext, concept: VideoConcept): Promise<ProductionPlan> {
    const result = await structuredResponse<ProductionPlan>({
      ...
      schema: {
        type: "object",
        additionalProperties: false,
        required: ["visual_bible", "scenes", "look_dev_prompts", "test_shot_indexes", "editing_strategy", "reuse_strategy", "production_notes"],
        properties: {
          ...
          scenes: {
            ...
            items: {
              type: "object",
              additionalProperties: false,
              required: ["title", "start_ms", "end_ms", "description", "visual_intent", "shots"],
              properties: {
                title: { type: "string" },
                start_ms: { type: "integer", minimum: 0 },
                end_ms: { type: "integer", minimum: 1 },
                ...
                shots: { type: "array", minItems: 1, items: SHOT_SCHEMA },
              },
            },
          },
          look_dev_prompts: {
          ...
          test_shot_indexes: { type: "array", items: { type: "integer", minimum: 0 }, maxItems: 4 },
  ...
  async reviseShot(input: {
    ...
    return structuredResponse<StoryboardShot>({
      ...
      prompt: JSON.stringify({
        ...
        current_shot: input.currentShot,
        visual_bible: input.visualBible,
        approved_concept: input.concept,
        project: compactContext(input.context),
      }),
      schema: SHOT_SCHEMA,
    });
  }
...
function validatePlanTimeline(plan: ProductionPlan, context: VideoProjectContext) {
  ...
  for (const shot of shots) {
    if (shot.end_ms <= shot.start_ms) throw new Error("Creative Director returned an invalid shot duration.");
  }
  if (durationMs > 0) {
    const tolerance = 1500;
    if (shots[0].start_ms > tolerance || Math.abs(shots.at(-1)!.end_ms - durationMs) > tolerance) {
      throw new Error("Creative Director storyboard does not cover the track duration closely enough. Regenerate the plan.");
    }
  }
}
