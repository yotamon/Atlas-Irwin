...
function scoreModel(model: HiggsfieldModelCapability, input: RouterInput) {
  ...
  if (input.targetResolution === "4k" && !model.supports4k) return -Infinity;
  ...
  if (profile.requires_audio_reference === true && model.supportsAudioReferences) score += 8;
  if (profile.requires_video_reference === true && model.supportsVideoReferences) score += 8;
...
export function routeVideoShot(input: RouterInput): ShotRoutingDecision {
  ...
  if (!winner) throw new Error("No video model satisfies the shot requirements.");
  ...
  if (winner.model.id === "seedance_2_5") params.mode = hasReferences(input) ? "omni_reference" : "t2v";
  if (winner.model.id === "kling3_0") params.mode = input.targetResolution === "4k" ? "4k" : input.generation_priority === "quality" ? "pro" : "std";
  ...
  return {
    ...
    params,
    reason: input.isTest
      ? `${winner.model.label} balances test cost with the required shot capabilities.`
      : `${winner.model.label} best matches the shot's ${input.generation_priority} priority and reference requirements.`,
  };
}
