...
export async function persistConceptRound(input: { db: SupabaseClient<VideoDatabase>; ownerId: string; project: ExtendedMusicVideoProject; concepts: VideoConcept[] }) {
  const { data: rounds, error: roundError } = await input.db.from("music_video_concepts").select("round_number").eq("project_id", input.project.id).eq("owner_id", input.ownerId).order("round_number", { ascending: false }).limit(1);
  if (roundError) throw new Error(roundError.message);
  const round = (rounds?.[0]?.round_number ?? 0) + 1;
  await input.db.from("music_video_concepts").update({ status: "superseded" }).eq("project_id", input.project.id).eq("owner_id", input.ownerId).eq("status", "draft");
  const { data, error } = await input.db.from("music_video_concepts").insert(input.concepts.map((concept, index) => ({ owner_id: input.ownerId, project_id: input.project.id, round_number: round, display_order: index, title: concept.title, premise: concept.premise, concept_data: json(concept), treatment: null, status: "draft" as const }))).select("*").order("display_order");
  if (error) throw new Error(error.message);
  await input.db.from("music_video_projects").update({ creative_generated_at: new Date().toISOString(), last_error: null, status: "concept_review" }).eq("id", input.project.id).eq("owner_id", input.ownerId);
  return data ?? [];
}
...
export async function persistProductionPlan(input: { db: SupabaseClient<VideoDatabase>; ownerId: string; context: VideoProjectContext & { project: ExtendedMusicVideoProject }; plan: ProductionPlan }) {
  ...
  const { error: clearScenesError } = await input.db.from("music_video_scenes").delete().eq("project_id", input.context.project.id).eq("owner_id", input.ownerId);
  ...
  for (const row of shotRows) {
    ...
    const { data, error } = await input.db.from("music_video_shots").insert({ ...row.data, scene_id: sceneIds[row.sceneIndex], continuity_from_shot_id: strategy === "unique" ? null : previousId }).select("id").single();
