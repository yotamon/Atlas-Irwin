...
async function findGeneratedAsset(db: SupabaseClient<VideoDatabase>, ownerId: string, artistId: string, generationRunId: string) {
  const { data, error } = await db.from("media_assets")
    .select("*")
    .eq("owner_id", ownerId)
    .contains("metadata", { marketing_generation_run_id: generationRunId, artist_id: artistId })
    .limit(1)
    .maybeSingle();
...
export async function storeRemoteMarketingAsset(input: {
  ...
  if (!asset) {
    ...
    const path = `${input.ownerId}/library/marketing/${input.artistId}/${input.generationRunId}.${extension}`;
    ...
    const { data, error } = await input.db.from("media_assets").insert({
      ...
      metadata: {
        ...
        title: `AI creative for ${input.context.release.title}`,
        description: "Generated by Ensemblis Creative Engine from artist-specific release and brand visual lineage.",
        tags: [`artist:${input.artistId}`, "ensemblis-generated", "marketing", "creative-lineage", "approval-required"],
    ...
    if (error) {
      if (error.code === "23505") {
        asset = await findGeneratedAsset(input.db, input.ownerId, input.artistId, input.generationRunId);
      } else {
  ...
  const { error: demoteError } = await musicDb.from("media_links")
  ...
  const { data: existingLink, error: existingLinkError } = await musicDb.from("media_links")
    .select("id")
    .eq("owner_id", input.ownerId)
    .eq("artist_id", input.artistId)
    .eq("content_item_id", input.contentItemId)
  ...
  const linkRow = {
    ...
    is_primary: true,
    caption: "Generated by Ensemblis Creative Engine",
  };
  const { error: linkError } = existingLink
    ? await musicDb.from("media_links").update(linkRow).eq("id", existingLink.id).eq("artist_id", input.artistId)
    : await musicDb.from("media_links").insert(linkRow);
  ...
  }).eq("id", input.contentItemId).eq("owner_id", input.ownerId).eq("artist_id", input.artistId);
