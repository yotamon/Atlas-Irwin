...
export async function loadAtlasCreativeDna(input: {
  ...
  const hardAntiPatterns = uniqueStrings([
    ...
    "chrome humanoids or anonymous glossy fashion models without an Atlas-specific reason",
  ...
  return {
    ...
    sourceHierarchy: [
      "real Atlas Irwin footage and photography",
      "canonical release artwork and approved alternate artwork",
      "approved Atlas brand references and previous accepted creative",
    ...
    evidenceSummary: approvedPatterns.length || rejectedPatterns.length
      ? `${approvedPatterns.length} accepted and ${rejectedPatterns.length} rejected recent creative outcome${approvedPatterns.length + rejectedPatterns.length === 1 ? "" : "s"} are available as creative-direction evidence.`
      : "No reviewed creative history yet; use the explicit brand world, release identity and approved reference assets as the source of truth.",
  };
}
