...
function serviceSupabase() {
  ...
  if (!key) {
    throw new Error(
      "SUPABASE_SERVICE_ROLE_KEY is required for SoundCloud token storage.",
    );
  }
...
function redirectUri(origin: string) {
  return (
    ...
    `${origin.replace(/\/$/, "")}/studio/soundcloud/callback`
  );
}

export function hasSoundCloudEnv() {
  return Boolean(
    process.env.SOUNDCLOUD_CLIENT_ID?.trim() &&
      process.env.SOUNDCLOUD_CLIENT_SECRET?.trim(),
  );
}
...
async function exchangeToken(body: URLSearchParams) {
  ...
  if (!response.ok) {
    throw new Error(
      `SoundCloud token exchange failed: ${await response.text()}`,
    );
  }
  return (await response.json()) as TokenResponse;
}

export async function completeSoundCloudOAuth({
  code,
  codeVerifier,
  origin,
  ownerId,
...
async function storeSoundCloudConnection(
  ...
  if (!soundCloudUserId || !profile.username) {
    throw new Error(
      "SoundCloud profile response was missing an id or username.",
    );
  }
...
async function validAccessToken(ownerId: string) {
  ...
  const token = data?.[0];
  if (!token)
    throw new Error("Connect SoundCloud before using the SoundCloud API.");
  if (
...
async function soundCloudApiFetch<T>(
  ownerId: string,
  endpoint: string,
  ...
  if (!response.ok) {
    throw new Error(
      `SoundCloud API request failed: ${response.status} ${await response.text()}`,
    );
  }
  return (await response.json()) as T;
}

async function soundCloudCollection<T>(
  ownerId: string,
  endpoint: string,
  maxPages = 4,
) {
...
function trackRow(ownerId: string, track: SoundCloudApiTrack) {
  ...
  if (!Number.isFinite(soundcloudId) || !track.title || !track.permalink_url) {
    throw new Error(
      "SoundCloud track response was missing id, title, or permalink_url.",
    );
  }
  return {
    owner_id: ownerId,
    ...
    favoritings_count: numberOrZero(
...
function playlistRow(ownerId: string, playlist: SoundCloudApiPlaylist) {
  ...
  if (
    !Number.isFinite(soundcloudId) ||
    !playlist.title ||
    !playlist.permalink_url
  ) {
    throw new Error(
      "SoundCloud playlist response was missing id, title, or permalink_url.",
    );
  }
  return {
    owner_id: ownerId,
...
async function promoteSoundCloudCatalog(
  ownerId: string,
  tracks: SoundCloudApiTrack[],
  playlists: SoundCloudApiPlaylist[],
) {
  const supabase = serviceSupabase();
  const [
    { data: releases, error: releaseLookupError },
    { data: studioTracks, error: trackLookupError },
  ] = await Promise.all([
    supabase
      .from("releases")
      .select("id,slug,title,story,artwork_url,soundcloud_url")
      .eq("owner_id", ownerId),
    supabase
      .from("tracks")
      .select("release_id,soundcloud_url")
      .eq("owner_id", ownerId),
  ]);
  if (releaseLookupError) throw new Error(releaseLookupError.message);
  if (trackLookupError) throw new Error(trackLookupError.message);
  const knownReleases = [...(releases ?? [])];
  const knownTracks = [...(studioTracks ?? [])];

  for (const playlist of playlists) {
    if (!playlist.id || !playlist.title || !playlist.permalink_url) continue;
    const playlistTracks = Array.isArray(playlist.tracks)
      ? playlist.tracks
      : [];
    const linkedCounts = new Map<string, number>();
    for (const track of playlistTracks) {
      const linked = knownTracks.find(
        (item) => item.soundcloud_url === track.permalink_url,
      );
      if (linked)
        linkedCounts.set(
          linked.release_id,
          (linkedCounts.get(linked.release_id) ?? 0) + 1,
        );
    }
    const linkedReleaseId = [...linkedCounts.entries()].sort(
      (a, b) => b[1] - a[1],
    )[0]?.[0];
    const existing = knownReleases.find(
      (release) =>
        release.soundcloud_url === playlist.permalink_url ||
        release.id === linkedReleaseId,
    );
    const trackCount = playlist.track_count ?? playlistTracks.length;
    const releaseType =
      trackCount <= 1 ? "Single" : trackCount <= 6 ? "EP" : "Album";
    const releaseValues = {
      title: existing?.title || playlist.title,
      slug: existing?.slug || `soundcloud-${playlist.id}`,
      release_type: releaseType,
      status: "Live",
      story: existing?.story || playlist.description || null,
      soundcloud_url: playlist.permalink_url,
      artwork_url: existing?.artwork_url || playlist.artwork_url || null,
      release_date: null,
    };
    const result = existing
      ? await supabase
          .from("releases")
          .update(releaseValues)
          .eq("id", existing.id)
          .select("id")
          .single()
      : await supabase
          .from("releases")
          .insert({ owner_id: ownerId, ...releaseValues })
          .select("id")
          .single();
    ...
    if (!existing)
      knownReleases.push({
        id: result.data.id,
        slug: releaseValues.slug,
        title: releaseValues.title,
        story: releaseValues.story,
        artwork_url: releaseValues.artwork_url,
        soundcloud_url: playlist.permalink_url,
      });
    const releaseId = result.data.id;
    const rows = playlistTracks
      .filter((track) => track.title && track.permalink_url)
      .map((track, index) => ({
        owner_id: ownerId,
        release_id: releaseId,
        title: track.title!,
        version: "SoundCloud catalog",
    ...
    if (rows.length) {
      const { error } = await supabase
        .from("tracks")
        .upsert(rows, { onConflict: "release_id,title" });
      if (error) throw new Error(error.message);
      knownTracks.push(
        ...rows.map((row) => ({
  ...
  for (const track of tracks) {
    if (!track.id || !track.title || !track.permalink_url) continue;
    if (knownTracks.some((item) => item.soundcloud_url === track.permalink_url))
      continue;
    const existing = knownReleases.find(
      (release) => release.soundcloud_url === track.permalink_url,
    );
    const releaseValues = {
      title: existing?.title || track.title,
      slug: existing?.slug || `soundcloud-${track.id}`,
      release_type: "Single",
      status: "Live",
      story: existing?.story || track.description || null,
      soundcloud_url: track.permalink_url,
      artwork_url: existing?.artwork_url || track.artwork_url || null,
      release_date: track.created_at?.slice(0, 10) || null,
    };
    const result = existing
      ? await supabase
          .from("releases")
...
export async function syncSoundCloudCatalog(ownerId: string) {
  const [tracks, playlists] = await Promise.all([
    soundCloudCollection<SoundCloudApiTrack>(
      ownerId,
      "/me/tracks?limit=50&linked_partitioning=true",
    ),
    soundCloudCollection<SoundCloudApiPlaylist>(
      ownerId,
      "/me/playlists?show_tracks=true&limit=50&linked_partitioning=true",
    ),
  ]);
  const supabase = serviceSupabase();
  if (tracks.length) {
  ...
  await promoteSoundCloudCatalog(ownerId, tracks, playlists);
  const { error } = await supabase
    .from("soundcloud_accounts")
...
export async function syncSoundCloudTrack(
  ownerId: string,
  soundcloudId: number,
) {
  const track = await soundCloudApiFetch<SoundCloudApiTrack>(
    ownerId,
    `/tracks/${soundcloudId}`,
  );
  ...
  const { data, error } = await serviceSupabase()
    ...
    .upsert(row, { onConflict: "owner_id,soundcloud_id" })
    .select("*")
    .single();
  if (error) throw new Error(error.message);
...
export function soundCloudSnapshot(
  track: Pick<
    SoundCloudTrack,
    "playback_count" | "favoritings_count" | "comment_count" | "reposts_count"
  >,
) {
  return {
