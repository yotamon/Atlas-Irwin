...
function safeFilename(value: string, fallback: string) { const cleaned = value.split(/[?#]/, 1)[0]?.split("/").pop()?.trim() ?? ""; return cleaned || fallback; }
...
function issueFromRevelator(input: Record<string, unknown>, storeId?: number): DistributionIssue {
  ...
  return { code: `revelator.${String(input.category ?? "metadata")}.${String(input.objectId ?? "release")}`, title: severity === "error" ? "Distribution requirement needs attention" : "Review distribution warning", detail: stripHtml(String(input.errorMessage ?? input.message ?? "The distribution provider reported a validation issue.")), severity, source: storeId ? "store" : "provider", objectType: ["release", "track", "artist"].includes(objectType) ? objectType as "release" | "track" | "artist" : "release", objectId: input.objectId == null ? undefined : String(input.objectId), storeId: storeId == null ? undefined : String(storeId) };
}
...
function recordArray(raw: unknown): Record<string, unknown>[] {
  ...
  for (const key of ["items", "data", "stores", "results"]) if (Array.isArray(body[key])) return (body[key] as unknown[]).filter((item): item is Record<string, unknown> => Boolean(item && typeof item === "object"));
  return [];
}
...
function lookupItem(item: Record<string, unknown>, kind: LookupKind): LookupItem | null {
  const ids: Record<LookupKind, unknown[]> = { store: [item.distributorStoreId, item.storeId, item.id], language: [item.languageId, item.id], musicStyle: [item.musicStyleId, item.styleId, item.id], contributorRole: [item.roleId, item.contributorRoleId, item.id] };
  const names: Record<LookupKind, unknown[]> = { store: [item.distributorStoreName, item.storeName, item.name], language: [item.languageCode, item.isoCode, item.code, item.name, item.languageName], musicStyle: [item.musicStyleName, item.styleName, item.name], contributorRole: [item.roleName, item.contributorRoleName, item.name] };
...
function writerRoleCandidates(role: ProviderCatalogWriter["role"]) { return role === "composer" ? ["composer"] : role === "lyricist" ? ["lyricist", "lyrics", "author"] : ["composer lyricist", "composer and lyricist", "songwriter"]; }
...
export class RevelatorProvider implements DistributionProvider {
  ...
  private tokenCacheKey() { return `${this.v1Base}|${this.partnerUserId}`; }
  private async getAccessToken(forceRefresh = false) {
    ...
    const response = await fetch(`${this.v1Base}/partner/account/login`, { method: "POST", cache: "no-store", headers: { Accept: "application/json", "Content-Type": "application/json" }, body: JSON.stringify({ partnerApiKey: this.partnerApiKey, partnerUserId: this.partnerUserId }) });
  ...
  private async request(base: string, path: string, init: RequestInit = {}, retried = false): Promise<unknown> {
    ...
    const response = await fetch(`${base}${path}`, { ...init, cache: "no-store", headers: { Authorization: `Bearer ${token}`, Accept: "application/json", ...(init.body && !isFormData ? { "Content-Type": "application/json" } : {}), ...init.headers } });
    if (response.status === 401 && !this.staticToken && !retried) { cachedProviderToken = null; return this.request(base, path, init, true); }
    const text = await response.text();
    let body: unknown = null;
    if (text) { try { body = JSON.parse(text); } catch { body = text; } }
    if (!response.ok) { const error = new Error(responseMessage(body, response.status)) as Error & { status?: number; body?: unknown }; error.status = response.status; error.body = body; throw error; }
    return body;
  }

  private async lookup(path: string, kind: LookupKind) { const raw = await this.request(this.v1Base, path); return recordArray(raw).map((item) => lookupItem(item, kind)).filter((item): item is LookupItem => item !== null); }
  ...
  private async uploadRemoteFile(url: string, filename: string, kind: "audio" | "cover") {
    ...
    const fileId = String(record.fileId ?? ""); const returnedFilename = String(record.filename ?? filename);
  ...
  private async artistExternalIds(profiles: ProviderCatalogRelease["artistProfiles"], stores: ProviderStore[]) {
    ...
    for (const profile of profiles) {
      ...
      const store = stores.find((candidate) => { const name = normalized(candidate.name); if (platform === "spotify") return name === "spotify" || name.startsWith("spotify "); if (platform === "apple music" || platform === "apple_music") return name.includes("apple music"); if (platform === "soundcloud") return name.includes("soundcloud"); return name === platform; });
  ...
  private async contributorPayload(contributors: ProviderCatalogContributor[], roles: LookupItem[]) {
    ...
    for (const contributor of contributors) { const role = requireLookupMatch(roles, contributor.role, "Contributor role"); result.push({ roleId: role.id, artist: { name: contributor.name } }); }
    return result;
  }
  ...
  private async writerPayload(writers: ProviderCatalogWriter[], roles: LookupItem[]) {
    return writers.map((writer) => {
      ...
      const role = writerRoleCandidates(writer.role).map((candidate) => { try { return requireLookupMatch(publishingRoles, candidate, "Writer role"); } catch { return null; } }).find((item): item is LookupItem => item !== null);
      ...
      return { composerName: writer.legalName, rightsId, roleId: role.id, share: writer.share, ...(rightsId === 2 ? { publisherId: 0, publisherName: writer.publisherName } : {}) };
    });
  }

  async listStores(): Promise<ProviderStore[]> {
    const raw = await this.request(this.v1Base, "/common/lookup/stores"); const stores: ProviderStore[] = [];
    for (const item of recordArray(raw)) { const parsed = lookupItem(item, "store"); if (!parsed) continue; stores.push({ id: parsed.id, name: parsed.name, active: item.isActive !== false && item.active !== false && String(item.status ?? "").toLowerCase() !== "disabled", category: item.category == null ? null : String(item.category), raw: item }); }
  ...
  async prepareRelease(input: ProviderCatalogRelease): Promise<ProviderPreparationResult> {
    ...
    const [languages, musicStyles, roles, stores] = await Promise.all([this.lookup("/common/lookup/languages", "language"), this.lookup("/common/lookup/musicstyles", "musicStyle"), this.lookup("/common/lookup/contributorRoles", "contributorRole"), this.listStores()]);
    ...
    const preparedTracks: Record<string, unknown>[] = []; const releaseContributors: Array<{ roleId: number; artist: { name: string } }> = [];
    for (const track of input.tracks) {
      ...
      const audioLanguage = requireLookupMatch(languages, track.audioLanguageCode, "Audio language"); const trackMetadataLanguage = requireLookupMatch(languages, track.metadataLanguageCode, "Track metadata language");
      ...
      const writers = await this.writerPayload(track.writers, roles); const writerShare = track.writers.reduce((sum, writer) => sum + writer.share, 0);
      ...
      preparedTracks.push({ artistName: track.artistName, artistExternalIds: externalIds, languageId: trackMetadataLanguage.id, audioLanguageId: audioLanguage.id, name: track.title, version: track.version || undefined, contributors, explicit: track.explicit, trackType: track.origin === "cover" ? 2 : track.origin === "public_domain" ? 3 : 1, trackProperties: track.includesAi ? [8] : [1], composerContentsDTO: writers, trackRecordingVersions: [{ recordingVersionType: 1, isrc: track.isrc || null, audioFiles: [{ audioId: audio.fileId, audioFilename: audio.filename, fileFormat }] }] });
    }
    ...
    const payload = { releaseId: requestedReleaseId, name: input.title, artistName: input.artistName, artistExternalIds: externalIds, contributors: releaseContributors, copyrightP: `${input.copyrightYear} ${input.productCopyrightHolder}`, copyrightC: `${input.copyrightYear} ${input.recordingCopyrightHolder}`, hasRecordLabel: Boolean(input.label?.trim()), ...(input.label?.trim() ? { labelName: input.label.trim() } : {}), previouslyReleased: input.previouslyReleased, ...(input.previouslyReleased && input.upc ? { upc: input.upc } : {}), ...(input.previouslyReleased && input.originalReleaseDate ? { releaseDate: input.originalReleaseDate } : {}), languageId: metadataLanguage.id, primaryMusicStyleId: musicStyle.id, releasesLocals: [], isCompilation: false, image: artwork, tracks: preparedTracks };
  ...
  async configureRelease(providerReleaseId: string, options: { releaseDate: string }) { return this.request(this.v1Base, "/content/release/retail/save", { method: "POST", body: JSON.stringify({ releaseId: Number(providerReleaseId), saleStartDate: options.releaseDate }) }); }
  ...
  async validateRelease(providerReleaseId: string, storeIds: number[] = []): Promise<ProviderValidationResult> {
    ...
    const body = raw && typeof raw === "object" ? raw as Record<string, unknown> : {}; const issues: DistributionIssue[] = [];
    ...
    for (const bucket of perStore) { if (!bucket || typeof bucket !== "object") continue; const typed = bucket as Record<string, unknown>; const storeId = Number(typed.distributorStoreId); const errors = Array.isArray(typed.errors) ? typed.errors : []; for (const issue of errors) if (issue && typeof issue === "object") issues.push(issueFromRevelator(issue as Record<string, unknown>, Number.isFinite(storeId) ? storeId : undefined)); }
    return { ready: body.hasBlockingErrors !== true && !issues.some((issue) => issue.severity === "error"), issues, raw };
  }

  async submitRelease(providerReleaseId: string, storeIds: number[]) { if (!storeIds.length) throw new Error("At least one music service must be selected before distribution."); await this.request(this.v1Base, `/distribution/release/addtoqueue?releaseId=${encodeURIComponent(providerReleaseId)}`, { method: "POST", body: JSON.stringify(storeIds) }); }

  async getDistributionStatus(providerReleaseId: string): Promise<ProviderDelivery[]> {
    const raw = await this.request(this.v1Base, `/distribution/release/all?pageNumber=1&pageSize=100&searchTerm=${encodeURIComponent(providerReleaseId)}`); const items = recordArray(raw); const release = items.find((item) => String(item.releaseId ?? item.id ?? "") === providerReleaseId); if (!release) return [];
    const distributions = release.distributionStatuses ?? release.stores ?? []; if (!Array.isArray(distributions)) return [];
    return distributions.filter((item): item is Record<string, unknown> => Boolean(item && typeof item === "object")).map((item) => ({ storeId: String(item.distributorStoreId ?? item.storeId ?? item.id ?? "unknown"), storeName: String(item.distributorStoreName ?? item.storeName ?? item.name ?? "DSP"), providerStatus: (item.status ?? item.distributionStatus ?? null) as string | number | null, url: item.url == null ? null : String(item.url), raw: item }));
  }
}

export function distributionProviderConfigured() { const staticToken = process.env.REVELATOR_ACCESS_TOKEN?.trim(); const partnerApiKey = process.env.REVELATOR_PARTNER_API_KEY?.trim(); const partnerUserId = process.env.REVELATOR_PARTNER_USER_ID?.trim(); return Boolean(staticToken || (partnerApiKey && partnerUserId)); }
export function getDistributionProvider(): DistributionProvider { const provider = (process.env.DISTRIBUTION_PROVIDER ?? "revelator").trim().toLowerCase(); if (provider === "revelator") return new RevelatorProvider(); throw new Error(`Unsupported distribution provider: ${provider}`); }
