...
export function deriveDistributionArtistState(input: {
  ...
  if (!input.release.title.trim() || !input.release.artist?.trim() || !input.release.genre?.trim()) {
    pushUnique(decisions, {
      ...
      title: "Complete the release identity",
      detail: "Title, artist and primary genre are required before delivery.",
      section: "release",
      severity: "required",
    });
  }
  if (!input.release.release_date) {
    pushUnique(decisions, {
      ...
      detail: "Stores need the canonical release date before Ensemblis can prepare delivery.",
      section: "release",
      severity: "required",
    });
  }
  if (!input.release.artwork_url && !input.release.cover_asset) {
    pushUnique(decisions, {
      key: "release.artwork",
      title: "Add the final cover artwork",
      detail: "Use the canonical release artwork that should appear on music services.",
      section: "release",
      severity: "required",
    });
  }
  if (!meta?.label_name.trim()) {
    pushUnique(decisions, {
      key: "release.label",
      title: "Confirm the label name",
      detail: "Use the imprint or self-release label name that should be delivered to stores.",
      section: "release",
      severity: "required",
    });
  }
  if (!meta?.product_copyright_line.trim() || !meta.recording_copyright_line.trim()) {
    pushUnique(decisions, {
      key: "release.copyright_lines",
      title: "Confirm the copyright lines",
      detail: "Add the product (C) and sound-recording (P) copyright identities exactly as they should be delivered.",
      section: "rights",
      severity: "required",
    });
  }
  if (meta?.upc_source === "artist" && !meta.upc) {
    pushUnique(decisions, {
      key: "release.upc",
      title: "Add the release UPC",
      detail: "You chose to supply the UPC, so the canonical code is required before submission.",
      section: "release",
      severity: "required",
    });
  }
  ...
  if (!input.tracks.length) {
    pushUnique(decisions, {
      key: "tracks.missing",
      title: "Add at least one mastered track",
      detail: "Distribution requires a release with canonical track audio.",
      section: "tracks",
      severity: "required",
    });
  }
  ...
  for (const track of input.tracks) {
    if (!track.audio_url) {
      pushUnique(decisions, {
        key: `track.${track.id}.master`,
        title: `Add the final master for ${track.title}`,
        detail: "Every delivered track needs canonical master audio.",
        section: "tracks",
        severity: "required",
      });
    }
    ...
    if (!trackMeta) {
      pushUnique(decisions, {
        key: `track.${track.id}.metadata`,
        title: `Confirm language and explicit status for ${track.title}`,
        detail: "Ensemblis needs the track language, explicit flag and origin before delivery.",
        section: "tracks",
        severity: "required",
      });
    }
    ...
    if (!trackWriters.length) {
      pushUnique(decisions, {
        key: `track.${track.id}.writers`,
        title: `Add writer credits for ${track.title}`,
        detail: "At least one legal composer/lyricist identity is required.",
        section: "credits",
        severity: "required",
      });
    } else if (Math.abs(writerShare(trackWriters) - 100) > 0.01) {
      pushUnique(decisions, {
        key: `track.${track.id}.shares`,
        title: `Make ${track.title} writer shares total 100%`,
        detail: `Current total is ${writerShare(trackWriters).toFixed(2)}%.`,
        section: "credits",
        severity: "required",
      });
    }
    if (!input.contributors.some((row) => row.track_id === track.id)) {
      pushUnique(decisions, {
        key: `track.${track.id}.contributors`,
        title: `Add a production credit for ${track.title}`,
        detail: "Add the producer, engineer or other canonical production contributor.",
        section: "credits",
        severity: "required",
      });
    }
  ...
  const requiredRights = [
    ...
    "samplesCleared",
    "contributorPermissionsConfirmed",
    "aiDeclarationConfirmed",
  ];
  if (!requiredRights.every((key) => rights[key] === true)) {
    pushUnique(decisions, {
      key: "rights.declarations",
      title: "Review and confirm release rights",
      detail: "Master, composition, samples, contributor permissions and AI provenance remain an explicit artist decision.",
      section: "rights",
      severity: "decision",
    });
  }
  ...
  if (vocals.clonedVoice === true && vocals.authorizationConfirmed !== true) {
    pushUnique(decisions, {
      key: "rights.voice_authorization",
      title: "Confirm synthetic voice authorization",
      detail: "A cloned or replicated voice cannot be submitted without explicit authorization.",
      section: "rights",
      severity: "decision",
    });
  }
  ...
  for (const platform of ["spotify", "apple_music"] as const) {
    ...
    if (!profile || !["confirmed", "create_new"].includes(profile.status)) {
      pushUnique(decisions, {
        ...
        title: `Confirm the ${platform === "spotify" ? "Spotify" : "Apple Music"} artist identity`,
        detail: "Choose the existing artist profile or explicitly create a new one to avoid catalog mis-mapping.",
        section: "profiles",
        severity: "decision",
      });
    }
  ...
  for (const issue of input.openIssues ?? []) {
    ...
    pushUnique(decisions, {
      ...
      title: issue.title,
      detail: issue.detail,
      section: issue.source === "store" || issue.source === "provider" ? "delivery" : "release",
      severity: issue.source === "provider" || issue.source === "store" ? "review" : "required",
    });
  }
...
export function canonicalDistributionMetadataSnapshot(meta: DistributionReleaseMetadata | null) {
  return meta ? {
