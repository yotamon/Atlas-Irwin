...
def _gain_for_loudness(lufs: float | None, target: float | None) -> float:
    if lufs is None or target is None:
        return 1.0
    gain_db = max(-3.0, min(3.0, target - lufs))
...
def _scan_peaks(path: Path) -> tuple[float, float]:
    ...
    true_peak = 0.0
    overlap = 4096
    previous = np.zeros((0, 2), dtype=np.float32)
    with sf.SoundFile(path) as handle:
        while True:
            ...
            joined = np.concatenate((previous, block), axis=0) if len(previous) else block
...
def render_plan(tracks: list[TrackDescriptor], plan: dict[str, Any], workdir: Path) -> tuple[Path, dict[str, Any]]:
    ...
    loudness_values = [track.loudness_lufs for track in tracks if track.loudness_lufs is not None]
    target_lufs = float(np.median(loudness_values)) if loudness_values else None
    ...
    def render_item(item: dict[str, Any]) -> np.ndarray:
        ...
        audio *= _gain_for_loudness(track.loudness_lufs, target_lufs)
    ...
    raw_peak, raw_true_peak, gain_reduction_db = _apply_ceiling(raw_path, final_path, -1.0)
    duration_ms = int(round(sf.info(final_path).duration * 1000.0))
    return final_path, {
        "duration_ms": duration_ms,
        "sample_rate": SAMPLE_RATE,
        "raw_peak": round(raw_peak, 6),
        "raw_true_peak_estimate": round(raw_true_peak, 6),
        "ceiling_dbtp": -1.0,
        "ceiling_gain_reduction_db": round(gain_reduction_db, 4),
        "loudness_match_target_lufs": round(target_lufs, 2) if target_lufs is not None else None,
