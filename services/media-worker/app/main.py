...
def analyze_audio_file(path: Path) -> dict[str, Any]:
    ...
    for index, (start_ms, end_ms) in enumerate(zip(boundaries[:-1], boundaries[1:])):
        ...
        sections.append({
            ...
            "start_ms": start_ms,
            "end_ms": end_ms,
            "energy": section_energies[index],
        })

    def onset_at(ms: int) -> float:
        frame = int(librosa.time_to_frames(ms / 1000, sr=sr, hop_length=hop))
        lo = max(0, frame - 3)
        hi = min(len(onset_norm), frame + 4)
        return float(np.max(onset_norm[lo:hi])) if hi > lo else 0.25

    edit_points = [
        {
            "ms": section["start_ms"],
            "confidence": round(0.55 + 0.44 * onset_at(section["start_ms"]), 3),
            "reason": f"Detected structural transition into {section['label'].lower()}.",
        }
