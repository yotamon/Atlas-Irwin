...
export function ReleaseForm({
  ...
  return (
    <form action={saveReleaseV2} className="studio-form studio-release-form-v2">
      ...
      <div className="studio-form-intro"><span className="section-label">Release essentials</span><h2>{release ? "What changed?" : `Give Ensemblis the minimum it needs for ${subject}`}</h2><p>Title, format and date are enough to create the workspace. Ensemblis manages the slug, workflow status, campaign window and release-relative schedule automatically.</p></div>
      ...
      {releaseDateLocked ? <div className="v2-provider-lock" role="status"><strong>Release date is locked by an external schedule</strong><span>At least one approved post is already scheduled at a connected provider. Change or cancel that provider schedule before moving the release date so Ensemblis and the external channel cannot drift apart.</span></div> : <div className="studio-smart-defaults" role="note"><strong>Ensemblis handles the rest</strong><span>A release plan and campaign phases are created automatically. If the release date moves, unlocked scheduled content moves with it. No paid AI generation runs from this form.</span></div>}
