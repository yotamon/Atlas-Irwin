...
function normalizeStage(stage: string): Stage {
  // Keep old URLs valid while the artist-facing information architecture moves
  // from internal process verbs to the release object itself.
  ...
  return STAGES.some(([key]) => key === stage) ? stage as Stage : "overview";
}
  ...
  return <div className="studio-v2-page release-workspace-v2 release-object-workspace ensemblis-release-mission">
    ...
    {activeStage === "overview" ? <div className="release-mission-overview">
      <section className="release-mission-hero" data-status={mission.status}>
        ...
        {nextMissionItem ? <Link className="button primary" href={nextMissionItem.href}>{nextMissionItem.title}</Link> : <Link className="button primary" href={`/studio/create?release=${release.id}`}>Create next asset</Link>}
      </section>
      ...
      {missionAttention.length ? <section className="today-v3-section">
        ...
        <div className="v2-inbox">{missionAttention.map((item) => <Link className="v2-inbox-item" href={item.href} key={item.key}><div><strong>{item.title}</strong><small>{item.detail}</small></div><b aria-hidden>→</b></Link>)}</div>
      ...
      {!release.artwork_url && !release.cover_asset ? <section className="v2-section" id="cover-upload"><div className="v2-section-heading"><div><span className="section-label">Release identity</span><h2>Add the cover artwork</h2></div></div><MediaUploader releaseId={release.id} defaultRole="cover" /></section> : null}
    ...
    {activeStage === "content" ? <div className="release-content-workspace">
      <section className="release-stage-intro-card v2-section"><div className="v2-section-heading"><div><span className="section-label">Content</span><h2>Make deliverables from the music, not content for content's sake</h2><p>Ensemblis carries the approved Moment, lyrics, stems, artist memory and release context into production automatically.</p></div><Link className="button primary" href={`/studio/create?release=${release.id}`}>Create something</Link></div></section>
      <div className="v2-create-grid release-deliverable-grid">
        <Link className="v2-create-card" href={`/studio/create?release=${release.id}`}><span className="section-label">Fast social creative</span><h2>Reels, lyric cuts and visual loops</h2><p>Choose the deliverable. Ensemblis picks the strongest musical source.</p><strong>Open Create →</strong></Link>
        <Link className="v2-create-card" href={`/studio/video?release=${release.id}`}><span className="section-label">Longer motion</span><h2>Video Director</h2><p>Build a coherent music-video world when the release needs more than a social cut.</p><strong>Direct video →</strong></Link>
        <Link className="v2-create-card" href={`/studio/production?release=${release.id}`}><span className="section-label">In progress</span><h2>Production</h2><p>Refine and approve only assets already connected to this release.</p><strong>Open production →</strong></Link>
      </div>
    ...
    {activeStage === "distribution" ? <section className="v2-section release-distribution-workspace">
      ...
      <div className="v2-publish-grid">
        <article><span>Music distribution</span><strong>DSP delivery</strong><small>Readiness, rights, provenance, stores and delivery status</small><Link href={`/studio/releases/${release.id}/distribution`}>Open delivery →</Link></article>
        <article><span>Listening destination</span><strong>{[release.spotify_url,release.soundcloud_url,release.youtube_url].filter(Boolean).length}/3 linked</strong><small>Spotify, SoundCloud and YouTube</small><Link href={`/studio/releases/${release.id}?view=advanced&tab=music`}>Manage destinations →</Link></article>
        <article><span>Artist web</span><strong>{release.publish_state === "live" ? "Live" : "Not live"}</strong><small>Public catalog and release presence</small><Link href={`/studio/releases/${release.id}?view=advanced&tab=website`}>Website controls →</Link></article>
        <article><span>Campaign publishing</span><strong>{releaseDateLocked ? `${providerScheduledCount} scheduled` : `${scheduled.length} ready`}</strong><small>{missingAsset.length ? `${missingAsset.length} still need assets` : "No asset blocker detected"}</small><Link href={`/studio/production?release=${release.id}`}>Open production →</Link></article>
      </div>
    </section> : null}

    {activeStage === "results" ? <section className="v2-section release-results-workspace">
      <div className="v2-section-heading"><div><span className="section-label">Results</span><h2>What the release is teaching us</h2><p>Numbers matter when they change the next decision. Ensemblis turns release evidence into a recommendation instead of leaving you with a dashboard to interpret.</p></div><Link href="/studio/growth?view=performance">Open performance</Link></div>
      ...
      <div className="release-result-insight"><span className="section-label">Ensemblis read</span><strong>{resultInsight(metrics)}</strong><div className="actions"><Link className="button primary" href="/studio/growth?view=opportunities">Review opportunities</Link><Link className="button" href="/studio/memory">What Ensemblis learned</Link></div></div>
    </section> : null}
  </div>;
}
