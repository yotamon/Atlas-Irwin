...
export default async function ArtistOnboardingPage() {
  ...
  return <div className="ensemblis-onboarding-shell"><OnboardingVisitTracker artistId={artist.artistId} /><header className="ensemblis-onboarding-brand"><span aria-hidden><EnsemblisMark /></span><strong>Ensemblis</strong></header><main className="ensemblis-onboarding-card">
    ...
    <section className="ensemblis-onboarding-action" aria-live="polite">
      ...
      {current === "moments" && mission && missionMoments.length ? <><span className="section-label">Your judgment</span><h2>Choose the musical Moments worth using</h2><p>Approve only the complete musical sections you would actually put in front of a listener.</p><Link className="button primary" href={href(`/studio/releases/${mission.id}?stage=create#moments`)}>Review {missionMoments.length} curated Moment{missionMoments.length === 1 ? "" : "s"}</Link></> : null}
      {current === "moments" && mission && !missionMoments.length ? <><span className="section-label">Working</span><h2>Curating the strongest musical sections</h2><p>The Release Mission exists and its Track Intelligence is being normalized into reusable Moments. No extra setup is required.</p><Link className="button primary" href={href(`/studio/releases/${mission.id}?stage=create#moments`)}>Open Release Mission</Link></> : null}
