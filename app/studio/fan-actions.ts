...
async function context(form: FormData) {
  ...
  return {
    db: supabase as unknown as SupabaseClient<FanGraphDatabase>,
    ownerId: user.id,
...
async function assertFan(db: SupabaseClient<FanGraphDatabase>, ownerId: string, artistId: string, fanId: string) {
  const { data, error } = await db.from("fan_profiles")
    .select("id")
    .eq("id", fanId)
    .eq("owner_id", ownerId)
...
export async function saveFanProfile(form: FormData) {
  ...
  const { error } = await db.from("fan_profiles").update({
    display_name: value(form, "display_name") || null,
    relationship_state: state,
  }).eq("id", fanId).eq("owner_id", ownerId).eq("artist_id", artistId);
  if (error) throw new Error(error.message);
  refresh(fanId);
}
...
export async function addVerifiedFanIdentity(form: FormData) {
  ...
  const { data: existing, error: existingError } = await db.from("fan_identities")
    .select("id,fan_id")
    .eq("artist_id", artistId)
    .eq("channel", channel)
    .eq("identifier_kind", identifierKind)
    .eq("external_subject_id", subject)
    .maybeSingle();
  if (existingError) throw new Error(existingError.message);
  if (existing && existing.fan_id !== fanId) {
    throw new Error("That verified identity already belongs to another relationship. Merge the two relationships only if you have explicit identity evidence.");
  }
  if (!existing) {
    const { error } = await db.from("fan_identities").insert({
      fan_id: fanId,
      owner_id: ownerId,
      artist_id: artistId,
      channel,
...
export async function saveFanPermission(form: FormData) {
  ...
  const { db, ownerId, artistId } = await context(form);
  await assertFan(db, ownerId, artistId, fanId);
  const { data: identity, error: identityError } = await db.from("fan_identities")
    .select("id,fan_id,channel,identifier_kind")
    .eq("id", identityId)
    .eq("fan_id", fanId)
    .eq("owner_id", ownerId)
    .eq("artist_id", artistId)
  ...
  if (status === "granted" && evidenceNote.length < 3) {
    throw new Error("Describe the consent evidence before recording permission as granted.");
  }
  const { error } = await db.from("fan_permissions").upsert({
    ...
    purpose,
    status,
    source: status === "revoked" ? "privacy_request" : "artist_record",
    evidence_at: status === "unknown" ? null : new Date().toISOString(),
    evidence_note: evidenceNote || null,
    expires_at: null,
  }, { onConflict: "identity_id,purpose" });
  if (error) throw new Error(error.message);
  refresh(fanId);
}
...
export async function mergeFanProfiles(form: FormData) {
  ...
  const { db, ownerId, artistId } = await context(form);
  await Promise.all([
    assertFan(db, ownerId, artistId, sourceFanId),
    assertFan(db, ownerId, artistId, targetFanId),
  ]);
  if (!["explicit_confirmation", "verified_contact_match", "provider_verified_link"].includes(evidenceType)) {
    throw new Error("Cross-channel merges require explicit or verified identity evidence.");
  }
  const { error } = await db.rpc("merge_fan_profiles", {
    p_source_fan_id: sourceFanId,
    p_target_fan_id: targetFanId,
