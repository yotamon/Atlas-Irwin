...
async function loadLookGeneration(projectId: string, generationId: string, ownerId: string) {
  const db = createServiceClient();
  const [{ data: project, error: projectError }, { data: generation, error: generationError }] = await Promise.all([
    db.from("music_video_projects").select("*").eq("id", projectId).eq("owner_id", ownerId).single(),
...
export async function rejectLookReference(form: FormData) {
  ...
  const { db, project, generation } = await loadLookGeneration(projectId, generationId, user.id);
  ...
  const nextMetadata = {
    ...
    look_rejected: true,
    look_rejection_reason: reason || "Rejected during look review",
