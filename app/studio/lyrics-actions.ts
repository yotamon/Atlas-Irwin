...
async function studioLyricsContext(form: FormData) {
  ...
  const { data: track, error } = await music.from("tracks")
    ...
    .single();
  if (error || !track) throw new Error(error?.message || "Track not found for the active artist.");
  return { db, user, artist, trackId, releaseId };
}
...
async function persistLyrics(form: FormData, status: LyricsStatus = "verified") {
  ...
  if (status !== "instrumental" && !sections.length) throw new Error("Ensemblis could not find any lyric lines to save.");
...
export async function analyzeTrackLyricsAction(form: FormData) {
  ...
  const { data: lyrics, error } = await base.db.from("track_lyrics")
    ...
    .eq("artist_id", base.artist.artistId)
    .maybeSingle();
  if (error) throw new Error(error.message);
