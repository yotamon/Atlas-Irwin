...
export default async function AnalyticsPage() {
  ...
  const [
    ...
    releasesResult,
    contentResult,
    legacyLearningsResult,
    campaignsResult,
    marketingLearningsResult,
  ] = await Promise.all([
    marketing.from("metric_snapshots").select("*").eq("owner_id", user.id).order("captured_at", { ascending: false }),
    ...
    supabase.from("release_learnings").select("*,releases(title)").eq("owner_id", user.id).order("created_at", { ascending: false }),
    marketing.from("campaigns").select("id,name,objective,primary_kpi,status").eq("owner_id", user.id).order("updated_at", { ascending: false }),
    marketing.from("marketing_learnings").select("*").eq("owner_id", user.id).order("created_at", { ascending: false }),
  ]);
  ...
  const ranked = content
    .map((item) => {
      ...
      return {
        ...item,
        aggregate,
        score: goalPerformanceScore(item.goal, aggregate),
        primarySignal: primarySignalValue(item.goal, aggregate),
      };
    })
    ...
    <>
      ...
      <section className="studio-panel feature">
        ...
        {ranked.length ? (
          <table className="studio-table">
            ...
            <tbody>{ranked.map((item) => <tr key={item.id}>
              <td><strong>{item.title}</strong><br /><small>{item.format}</small></td>
              <td>{item.goal}</td>
              <td>{item.platform}</td>
              <td>{item.goal === "Reach" ? Math.round(item.primarySignal).toLocaleString() : formatRate(item.primarySignal)}</td>
              <td>{item.score}</td>
            </tr>)}</tbody>
          </table>
      ...
      <section className="studio-panel feature">
        ...
        {campaigns.length ? campaigns.map((campaign) => (
          <Link className="list-row" href={`/studio/campaigns/${campaign.id}`} key={campaign.id}>
            <span><strong>{campaign.name}</strong><br /><small>{campaign.objective} / KPI: {campaign.primary_kpi}</small></span>
            <Status>{campaign.status}</Status>
          </Link>
        )) : <EmptyState title="No campaigns yet" body="Create a first-class campaign to connect content, experiments, attribution and learnings." href="/studio/campaigns#new" label="Create campaign" />}
      </section>
      ...
      <section className="studio-panel feature">
        ...
        {marketingLearnings.length ? marketingLearnings.map((learning) => (
          <article className="list-row" key={learning.id}>
            <div><strong>{learning.finding}</strong><br /><small>{learning.scope} / {Math.round(Number(learning.confidence) * 100)}% confidence / {learning.source}</small></div>
            {learning.status === "proposed" ? <div className="actions">
              <form action={setLearningStatus}><input type="hidden" name="learning_id" value={learning.id} /><input type="hidden" name="status" value="approved" /><button className="button primary" type="submit">Approve</button></form>
            ...
            </div> : <Status>{learning.status}</Status>}
          </article>
        )) : <EmptyState title="No structured learnings yet" body="When an experiment clears its sample and lift threshold, Atlas proposes a learning here. It is never promoted to future planning context without approval." />}
      </section>
      ...
      <section className="studio-panel feature">
        ...
        <form action={saveLearning} className="studio-form">
          <div className="form-grid">
            <Field label="Release"><select name="release_id" required><option value="">Select release</option>{releases.map((release) => <option value={release.id} key={release.id}>{release.title}</option>)}</select></Field>
            <Field label="Learning"><input name="learning" required placeholder="Manual observation worth preserving" /></Field>
          </div>
          <Submit>Save manual learning</Submit>
        </form>
      </section>

      {metrics.length ? (
        <section className="studio-panel feature">
          <div className="panel-head"><h2>Snapshot history</h2></div>
          <table className="studio-table">
            <thead><tr><th>Date</th><th>Platform</th><th>Views</th><th>Follows</th><th>Source</th><th></th></tr></thead>
            <tbody>{metrics.map((metric) => <tr key={metric.id}><td>{metric.date}</td><td>{metric.platform}</td><td>{metric.views}</td><td>{metric.follows}</td><td>{metric.source}</td><td><form action={deleteStudioRecord}><input type="hidden" name="id" value={metric.id} /><input type="hidden" name="table" value="metric_snapshots" /><button className="text-button">Delete</button></form></td></tr>)}</tbody>
          </table>
        </section>
      ) : null}
    </>
