...
import { completeTask, deleteTask, saveTask } from "@/app/studio/task-actions";
import {
  EmptyState,
  Field,
  PageHeader,
...
export default async function TasksPage({
  ...
  const [{ data: tasks, error: tasksError }, { data: releases, error: releasesError }] = await Promise.all([
    query,
    music
      .from("releases")
      .select("id,title")
    ...
    <>
      ...
      <div className="studio-grid task-overview">
        <section className="studio-panel">
          <div className="panel-head"><h2>Open</h2></div>
          <p className="metric-figure">{open.length}</p>
        </section>
        <section className="studio-panel">
          <div className="panel-head"><h2>Due soon</h2></div>
        ...
        <section className="studio-panel">
          <div className="panel-head"><h2>Done</h2></div>
      ...
      {!tasks?.length ? (
        ...
        <section className="task-list" aria-label="Tasks">
          {tasks.map((task) => {
            ...
            return (
              <article className={`task-row ${task.status === "Done" ? "done" : ""}`} id={`task-${task.id}`} key={task.id}>
                <div>
                  <Status>{task.priority}</Status>
                  <Status>{task.status}</Status>
                  <h3>{task.title}</h3>
                  <p>
                    {release ? <Link href={`/studio/releases/${release.id}`}>{release.title}</Link> : "No release"}
                    {task.due_at
                      ? ` · due ${new Intl.DateTimeFormat("en", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" }).format(new Date(task.due_at))}`
                      : " · no due date"}
                  </p>
                </div>
                  ...
                  <details>
                    ...
                    <form action={saveTask} className="studio-form">
                      ...
                      <div className="form-grid">
                        ...
                        <Field label="Release">
                          <select name="release_id" defaultValue={task.release_id ?? ""}>
                            <option value="">No release</option>
                            {releases?.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}
                          </select>
                        </Field>
                        <Field label="Due"><input type="datetime-local" name="due_at" defaultValue={toLocalInput(task.due_at)} /></Field>
                      </div>
      ...
      <section id="new" className="studio-panel feature">
        ...
        <form action={saveTask} className="studio-form">
          ...
          <div className="form-grid">
            ...
            <Field label="Due"><input type="datetime-local" name="due_at" /></Field>
          </div>
