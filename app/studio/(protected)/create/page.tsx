...
export default async function CreatePage({
  ...
  searchParams: Promise<{ track?: string; release?: string }>;
}) {
  ...
  const otherStartingPoints = [
    { title: "Add or create music", description: "Bring in a master or create a musical draft when the source music does not exist yet.", href: href("/studio/music?view=add") },
    ...
    { title: "Direct a video", description: "Use Video Director for a larger coherent music video.", href: href(activeRelease ? `/studio/video?release=${activeRelease.id}` : "/studio/video") },
  ];
  ...
  return (
    <div className="studio-v2-page create-polish-page">
      <PageHeader
        title="Create"
        description={requestedTrack ? `Creative directions from ${requestedTrack.title}.` : `Start from the strongest musical Moments for ${artist.artistName}.`}
      ...
      {directions.length ? (
        <section className="v2-section">
          <div className="v2-section-heading">
            <div><span className="section-label">Recommended musical starting points</span><h2>Choose the idea</h2></div>
            {activeRelease ? <Link href={href(`/studio/releases/${activeRelease.id}?stage=create#moments`)}>Review source Moments</Link> : null}
          </div>
          <p className="v2-muted-copy">Choose from the three strongest creative directions. Each is an evidence-backed musical starting point from an approved Moment.</p>
            ...
            {directions.map((direction) => {
              ...
              return (
                <article className="growth-opportunity accepted create-moment-card" key={direction.id}>
                  ...
                  <small>{release?.title || "Release"} · {track?.title || "Track"} · {momentTime(moment.start_ms)}–{momentTime(moment.end_ms)}</small>
                  ...
                  <div className="actions create-evidence-action"><Link className="button" href={href(`/studio/releases/${moment.release_id}?stage=create#moments`)}>Inspect evidence</Link></div>
                </article>
              );
            })}
          </div>
        ...
        <section className="v2-section"><div className="v2-calm-state compact"><strong>No approved musical Moment is ready yet.</strong><p>Add and analyze music first. Ensemblis will wait for real musical evidence.</p><Link className="button primary" href={href("/studio/music?view=add")}>Add music</Link></div></section>
      )}

      <aside className="create-next-action-callout">
        <div><span className="section-label">Not sure what to make?</span><strong>Start with the first ranked direction.</strong><p>Today owns priority. Create only asks you to choose a musically grounded idea.</p></div>
