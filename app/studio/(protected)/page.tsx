...
export default async function TodayPage() {
  ...
  const [releasesResult, tracksResult, campaignsResult, tasksResult, automationResult, publicationResult, contentResult, learningsResult, nextActionsResult, soundCloudPendingResult, spotifyPendingResult, outreachDraftsResult] = await Promise.all([
  ...
  const publicationContentIds = new Set(scheduledPublications.map((job) => job.content_item_id).filter(Boolean));
  const upcomingContent = content.filter((item) => item.scheduled_at && new Date(item.scheduled_at) >= now && new Date(item.scheduled_at) <= sevenDays && !publicationContentIds.has(item.id));
  const comingUp: ComingUpItem[] = [
    ...scheduledPublications.map((job) => ({ id: `scheduled-publication-${job.id}`, title: `${job.platform} publication`, detail: "Scheduled and ready to move", scheduledAt: job.scheduled_at!, href: href(job.content_item_id ? `/studio/production?edit=${job.content_item_id}` : "/studio/growth") })),
    ...upcomingContent.map((item) => ({ id: `scheduled-content-${item.id}`, title: item.title, detail: `${item.platform} content`, scheduledAt: item.scheduled_at!, href: href(`/studio/production?edit=${item.id}`) })),
  ].sort((left, right) => new Date(left.scheduledAt).getTime() - new Date(right.scheduledAt).getTime()).slice(0, 7);
