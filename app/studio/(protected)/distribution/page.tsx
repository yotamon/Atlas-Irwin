...
export default async function DistributionHub({
  ...
  const [accountResult, releasesResult, configsResult, issuesResult, deliveriesResult] = await Promise.all([
    ...
    db.from("release_read_model").select("id,title,artist_name,release_type,release_date,cover_public_url,cover_asset_id,status,is_archived").eq("owner_id", user.id).eq("artist_id", artist.artistId).eq("is_archived", false).order("release_date", { ascending: false, nullsFirst: false }),
