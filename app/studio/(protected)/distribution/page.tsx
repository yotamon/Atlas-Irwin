...
export default async function DistributionHub({
  ...
  const deliveryByRelease = new Map<string, any[]>();
  ...
  const activeConfigs = configs.filter((config: any) => !["draft", "needs_attention", "ready"].includes(config.state));
  const liveCount = configs.filter((config: any) => config.state === "live").length;
  const inFlightCount = configs.filter((config: any) => ["submitted", "under_review", "approved", "delivering", "delivered", "partially_live"].includes(config.state)).length;
  const needsAttentionCount = configs.filter((config: any) => ["needs_attention", "rejected", "error"].includes(config.state)).length;
  ...
  return <div className="distribution-page distribution-hub">
    ...
    <section className="distribution-section">
      ...
      <div className="distribution-release-list">
        {releases.map((release) => {
          const config = configByRelease.get(release.id) as any;
    ...
    {issues.length ? <section className="distribution-section distribution-issues"><div className="distribution-section-heading"><div><span className="section-label">Portfolio attention</span><h2>Open distribution findings</h2></div></div><div className="distribution-issue-list">{issues.map((issue: any) => { const release = releases.find((item) => item.id === issue.release_id); return <Link href={`/studio/releases/${issue.release_id}/distribution`} className={`issue-${issue.severity}`} key={issue.id}><span>{issue.severity}</span><div><strong>{issue.title}</strong><p>{issue.detail}</p><small>{release?.title ?? "Release"}</small></div></Link>; })}</div></section> : null}
