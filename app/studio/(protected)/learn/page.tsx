...
export default async function LearnPage() {
  ...
  const [metricsResult, contentResult, learningsResult, campaignsResult] = await Promise.all([
    ...
    marketing.from("campaigns").select("id,name,status,objective,primary_kpi").eq("owner_id", user.id).order("updated_at", { ascending: false }),
  ]);
  ...
  return (
    <div className="studio-v2-page">
      <PageHeader title="Learn" description="Atlas turns performance into reusable decisions. Manual data entry exists only as an advanced fallback." action={<Link className="button" href="/studio/analytics">Advanced analytics</Link>} />
      ...
      <section className="v2-section">
        ...
        {proposed.length ? <div className="v2-learning-review">{proposed.map((learning) => (
          <article key={learning.id}>
            <div><span>{learning.scope} · {Math.round(Number(learning.confidence) * 100)}% confidence</span><strong>{learning.finding}</strong><small>Source: {learning.source}. Atlas will not reuse this conclusion until you approve it.</small></div>
        ...
        ))}</div> : <div className="v2-calm-state compact"><strong>Atlas has no uncertain conclusions for you.</strong><p>Reliable experiment results will appear here instead of becoming hidden configuration.</p></div>}
      </section>
      ...
      <section className="v2-section v2-compact-section">
        ...
        <p className="v2-muted-copy">Manual metric entry and legacy release notes are still available in Advanced analytics, but they are no longer part of the normal workflow.</p>
      </section>
    </div>
  );
}
