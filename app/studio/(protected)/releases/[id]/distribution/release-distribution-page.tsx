...
export default async function ReleaseDistributionPage({
  ...
  const [releaseResult, tracksResult, configResult, accountResult, profilesResult, issuesResult, deliveriesResult, submissionsResult, metadataResult, writersResult, contributorsResult, operationsResult] = await Promise.all([
    ...
    db.from("distribution_validation_issues").select("*").eq("release_id", id).eq("owner_id", user.id).eq("artist_id", artist.artistId).in("status", ["open", "acknowledged"]).order("severity"),
    db.from("distribution_deliveries").select("*").eq("release_id", id).eq("owner_id", user.id).eq("artist_id", artist.artistId).order("store_name"),
    db.from("distribution_submissions").select("id,version,submitted_at,provider,provider_release_id,state").eq("release_id", id).eq("owner_id", user.id).eq("artist_id", artist.artistId).order("version", { ascending: false }).limit(8),
    ...
    db.from("distribution_provider_operations").select("*").eq("release_id", id).eq("owner_id", user.id).eq("artist_id", artist.artistId).order("created_at", { ascending: false }).limit(12),
  ]);
  ...
  return <div className="distribution-page distribution-release-workspace">
    <header className="distribution-header">
      <div><Link className="v2-back-link" href={`/studio/releases/${release.id}?stage=publish`}>← {release.title}</Link><div className="distribution-title-row"><div><span className="section-label">Ensemblis Distribution · {artist.artistName}</span><h1>Music distribution</h1></div><span className={`distribution-state state-${state}`}>{stateLabel(state)}</span></div><p>Complete one canonical release package, validate it against store requirements, approve delivery, then monitor every destination.</p></div>
    ...
    <section className="distribution-section">
      ...
      <form action={saveDistributionDeclarations} className="distribution-form">{artistInput}<input type="hidden" name="release_id" value={release.id} /><fieldset disabled={locked}>
