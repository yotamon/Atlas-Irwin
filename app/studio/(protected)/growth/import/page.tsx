...
export default async function GrowthImportPage() {
  ...
  const { data: recent, error } = await growth.from("track_vault").select("*")
  ...
  return <div className="studio-v2-page growth-import-page">
    <PageHeader title="Import unreleased masters" description={`Drop ${artist.artistName}'s backlog in one pass. Each audio file becomes a reusable Media Library asset and an independent Vault track; free worker analysis starts automatically when configured.`} action={<Link className="button" href="/studio/growth#vault">Back to Vault</Link>} />
    ...
    <section className="v2-section">
      ...
      <p className="v2-muted-copy">Do not create releases yet. Upload masters, let Ensemblis extract structural signals, then rate only the subjective things the computer cannot know, such as how much you personally believe in the track and how distinctive it feels in this artist catalog.</p>
